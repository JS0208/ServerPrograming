...
USE backtest_db;
ALTER TABLE results
ADD COLUMN cagr FLOAT NULL COMMENT 'Compound Annual Growth Rate (%)' AFTER win_rate;

USE backtest_db;
ALTER TABLE results
ADD COLUMN parameters TEXT NULL COMMENT 'JSON string of backtest parameters' AFTER cagr;

-- 예시: strategies 테이블에 user_id 추가
ALTER TABLE strategies
ADD COLUMN user_id INT NULL AFTER description, -- 또는 적절한 위치
ADD CONSTRAINT fk_strategy_user
    FOREIGN KEY (user_id)
    REFERENCES users(id)
    ON DELETE SET NULL; -- 또는 ON DELETE CASCADE (사용자 삭제 시 전략도 삭제)

-- 예시: results 테이블에 user_id 추가
ALTER TABLE results
ADD COLUMN user_id INT NULL AFTER strategy_id, -- 또는 적절한 위치
